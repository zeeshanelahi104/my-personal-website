        ...
        <div className="w-full md:w-1/3 flex flex-col justify-center gap-4 md:gap-8">
          ...
          {infoData?.map((item) => (
            <div className="flex items-center space-x-4" key={item?.title}>
              <span className="bg-lightSky/10 p-3 border rounded-lg border-hoverColor/10 text-hoverColor">{item?.icon}</span>

              <div >
                <h3 className="text-white/60 font-semibold">{item?.title}</h3>
                <p className="text-white/60 whitespace-nowrap">{item?.details}</p>
              </div>
            </div>
          ))}
        </div>
